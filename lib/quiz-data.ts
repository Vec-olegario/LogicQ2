...
export const BANCO_PERGUNTAS: Question[] = [
  // ── RECEBIMENTO ──
  {
...
export function getPerguntasByTopico(topico: string): number[] {
  return BANCO_PERGUNTAS.reduce((acc, p, idx) => {
    if (p.topico === topico) acc.push(idx);
    return acc;
  }, [] as number[]);
}

export function getAllPerguntasIndices(): number[] {
  return BANCO_PERGUNTAS.map((_, i) => i);
}

export function getPerguntaByIndex(idx: number): Question | null {
  if (idx >= 0 && idx < BANCO_PERGUNTAS.length) {
